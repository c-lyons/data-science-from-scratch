...
class NaiveBayes(object):
    ...
    def predict(self, X):
        ...
        for c, gauss in self.gaussians.items():
            mean, var = gauss['mean'], gauss['var']  # extracting gauss, var from gaussian dict for class c
            P[:, c] = multivariate_normal.logpdf(X, mean=mean, cov=var, allow_singular=True) + np.log(self.priors[c])  # using logs so additive probs and not mult
